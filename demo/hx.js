...
function onHx(e) {
    ...
    if (el) {
        if (!el.getAttribute("hx-projection"))
            return;
        e.preventDefault();
...
async function composeResponse(e, abortSignal) {
    if (!(e.target instanceof Element))
        return;
    if (!e.target.getAttribute("hx-projection"))
        return;
    ...
    if (hxResponse.response)
        e.target.setAttribute("hx-status-code", hxResponse.response.status.toString());
...
export { HxProjectEvent, HxRequestEvent, HxResponseEvent, connect, disconnect };
